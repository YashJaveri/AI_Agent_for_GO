...
def max_value(go, num_dead_pieces_me, num_dead_pieces_enemy, alpha, beta, depth):
    ...
    for i in range(board_size):
        for j in range(board_size):
            ...
            if go_clone.place_chess(i, j, piece_type):
                ...
                x = min_value(go_clone, num_dead_pieces_me, num_dead_pieces_enemy, alpha, beta, depth)
                num_dead_pieces_enemy -= len(dead_pieces_enemy)
                if x>alpha:
                    alpha = x
...
def min_value(go, num_dead_pieces_me, num_dead_pieces_enemy, alpha, beta, depth):
    if depth == 0:        
        return evaluator(go, num_dead_pieces_me, num_dead_pieces_enemy)
    no_moves = True
    for i in range(board_size):
        for j in range(board_size):
            ...
            if go_clone.place_chess(i, j, 3-piece_type):
                ...
                x = max_value(go_clone, num_dead_pieces_me, num_dead_pieces_enemy, alpha, beta, depth-1)
                num_dead_pieces_me -= len(dead_pieces_me)     
                if x<beta:
                    beta = x
                if alpha>=beta:
...
def get_best_move(go):
    ...
    _ = max_value(go, 0, 0, alpha_initial, beta_initial, min_max_depth)                    
